        ...
        <div class="row mt-5">
            <div class="col-md-12">
                <table id="exampl" class="table cell-border " style="width:100%">
                    <h2 style="color: #9f8e64;">Patient history</h2><br>
                    <?php
                    $patient_data = $dbObj->connFnc()->query("select * from `tbl_login` INNER JOIN `tbl_patient` on `tbl_login`.`l_id` = `tbl_patient`.`l_id` WHERE `tbl_login`.`l_id` = '" . $_GET['patient_id'] . "';")->fetch_assoc();
                    $count = $dbObj->connFnc()->query("SELECT count(*) as total_appo FROM `appoinment_tbl` INNER JOIN `tbl_patient` ON `appoinment_tbl`.`l_id` = `tbl_patient`.`l_id` INNER JOIN `doctor_timing_tbl` on `appoinment_tbl`.`time_id` = `doctor_timing_tbl`.`time_id` LEFT JOIN `tbl_login` ON `doctor_timing_tbl`.`l_id` = `tbl_login`.`l_id` WHERE `appoinment_tbl`.`l_id` = '" . $_GET['patient_id'] . "' AND `appoinment_tbl`.`status` = 3;")->fetch_assoc()['total_appo'];
                    // print_r($patient_data)
                    ?>
                    <h6>Patient Name : <?= $patient_data['u_name'] ?></h6>
                    <h6>Address : <?= $patient_data['address'] . ',' . $patient_data['city'] ?></h6>
                    <h6>Total Appoinments : <?= $count ?></h6>
                    <h6>Gender : <?= $patient_data['gender'] ?></h6>
                    <h6> Blood group : <?= $patient_data['bloodgrp'] ?></h6>
